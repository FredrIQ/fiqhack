...
NetHack 4 - dungeon exploration game
...
NetHack 4 is a "roguelike" computer game in which you control an
...
Because this option is parsed by libuncursed, not NetHack 4 proper, it
...
Overrides the game playfield (where NetHack 4 stores data common to
all users) or the user directory (where NetHack 4 stores data specific
to an individual user), respectively.  This is intended for use with
L<dgamelaunch(8)>.
...
L<hack(6)>, L<rogue(6)>, L<nethack(6)>, L<http://nethack4.org>

=head1 BUGS

Numerous.  If you find a bug, or want to peruse the list of bugs that
other players have found, see the bug tracker at
L<http://trac.nethack4.org>.
