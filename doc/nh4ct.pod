...
replaced, when compiling, with Unicode character of the most recently seen
...
possible backwards compatibility; for two-letter keys, using C<__> as the first
...
specific palette (especially when the C<-p> option, to lock the palette to that
