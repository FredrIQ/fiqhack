...
One other issue was that the code for rendering ASCII/Unicode and for
rendering tiles had diverged dangerously, meaning two different sets of code
needed to be supported.  The method of rendering substitutions (such as dark
rooms and lit corridors), in particular, was fragile and buggy.  Thus, it
...
replaced, when compiling, with the Unicode character of the most recently seen
I<unsubstituted> tile that matches the tile being compiled.  For instance, if
you wanted statues to be gray versions of the corresponding monster, you could
write a rule like C<sub statue *: gray basechar> (so long as the monsters had
already been defined at that point, either in that tileset or in a tileset
that came earlier on the command line).  This is numerically represented using
the (invalid) Unicode character 0xd800.
...
bright_magenta; other colors shift to brown
...
C<!>, that can appear anywhere except inside an image definition; these are
ignored.
...
long (although all keys in any given file must have the same length).  For
single-letter keys, avoiding underscores and dollars gives the best possible
backwards compatibility; for two-letter keys, using C<__> as the first key is
recommended by the Slash'EM tiles developers to get old versions of the tiles
utilities to bail out cleanly.  Providing a palette with no tiles is useful
when doing tiles merging, in order to adjust all the tiles onto a specific
palette (especially when the C<-p> option, to lock the palette to that in a
particular file, is given).
...
braces (C<{>, C<}>); each row of the image is one line of text, specifying all
...
to give the red, green, blue and alpha separately.  3-channel and 4-channel
keys can be mixed, but neither can be mixed with a 1-channel key.)  The braces
are each on lines of their own, and no colon is used.
...
The behaviour of patterns with substitution tiles is unsatisfactory, and needs
to be improved.

There's currently no way to do batch image processing, such as darkening floor
tiles to implement C<sub unlit>.
