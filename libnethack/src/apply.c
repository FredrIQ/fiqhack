...
use_camera(struct obj *obj, const struct nh_cmd_arg *arg)
{
    ...
    if (obj->cursed && !rn2(2))
        ...
        pline(msgc_yafm, "You take a picture of the %s.",
              (dz > 0) ? surface(youmonst.mx, youmonst.my) : ceiling(youmonst.mx, youmonst.my));
    else
        ...
        *resp = 1;
        ...
        if (otmp->otyp == CORPSE) {
            pline(msgc_yafm, "You determine that %s unfortunate being is dead.",
                  (rx == youmonst.mx && ry == youmonst.my) ? "this" : "that");
        } else {
    ...
    if (u.usteed && dz > 0) {
        ...
        if (Underwater)
            ...
            pline(msgc_cancelled1, "You can't reach the %s.",
                  (dz > 0) ? surface(youmonst.mx, youmonst.my) : ceiling(youmonst.mx, youmonst.my));
        else if (its_dead(youmonst.mx, youmonst.my, &res))
...
{
    return (boolean) (abs(youmonst.mx - x) > n || abs(youmonst.my - y) > n);
}
...
check_leash(xchar x, xchar y)
{
    ...
    for (otmp = youmonst.minvent; otmp; otmp = otmp->nobj) {
        ...
        if (dist2(youmonst.mx, youmonst.my, mtmp->mx, mtmp->my) >
...
use_mirror(struct obj *obj, const struct nh_cmd_arg *arg)
{
    ...
    schar sx = youmonst.mx;
    schar sy = youmonst.my;
    int range = COLNO;
...
use_bell(struct obj **optr)
{
    ...
    boolean invoking = obj->otyp == BELL_OF_OPENING &&
        invocation_pos(&u.uz, youmonst.mx, youmonst.my) && !On_stairs(youmonst.mx, youmonst.my);
    ...
    if (Underwater || (Engulfed && ordinary)) {
        ...
        if (obj->cursed && !rn2(4) &&
            ...
            ((mtmp = makemon(mkclass(&u.uz, S_NYMPH, 0, rng_main), level,
                             youmonst.mx, youmonst.my, NO_MINVENT)))) {
...
use_candelabrum(struct obj *obj)
{
    ...
    if (!invocation_pos(&u.uz, youmonst.mx, youmonst.my) ||
        On_stairs(youmonst.mx, youmonst.my)) {
...
catch_lit(struct obj * obj)
{
    ...
    if (!obj->lamplit && (obj->otyp == MAGIC_LAMP || ignitable(obj))) {
        ...
        if (obj->otyp == POT_OIL)
            tell_discovery(obj);
        if (obj->unpaid && costly_spot(youmonst.mx, youmonst.my) &&
              ...
              boolean checking)
{
    ...
    if (!magic && (nolimbs(mon->data) || slithy(mon->data))) {
        bpline(show, msgc_cancelled, "You can't jump; you have no legs!");
        return 0;
    } else if (!magic && !trinsic) {
        bpline(show, msgc_cancelled, "You can't jump very far.");
        return 0;
    } else if (you && Engulfed) {
        bpline(show, msgc_cancelled, "You've got to be kidding!!");
        return 0;
    } else if (m_underwater(mon)) {
        bpline(show, msgc_cancelled, "This calls for swimming, not jumping!");
        return 0;
    } else if (mon == u.ustuck || (you && u.ustuck)) {
        if (u.ustuck->mtame && !Conflict && !confused(u.ustuck)) {
            if (!checking) {
                bpline(show, msgc_actionok, "%s free from %s.",
            ...
            /* TODO: this spoils something (no conflict/pet confusion) under
               ...
               msgc_cancelled */
            bpline(show, msgc_cancelled, "You cannot escape from %s!",
                   mon_nam(u.ustuck));
            return 0;
        }
    } else if (levitates(mon) || Is_airlevel(m_mz(mon)) ||
               Is_waterlevel(m_mz(mon))) {
        bpline(show, msgc_cancelled, "You don't have enough traction to jump.");
        return 0;
    } else if (!magic && you && near_capacity() > UNENCUMBERED) {
        bpline(show, msgc_cancelled, "You are carrying too much to jump!");
        return 0;
    } else if (!magic && you && (u.uhunger <= 100 || ACURR(A_STR) < 6)) {
        bpline(show, msgc_cancelled, "You lack the strength to jump!");
        return 0;
        ...
        if (leg_hurtl(maybe_steed) && leg_hurtr(maybe_steed))
            bp = makeplural(bp);
        if (u.usteed)
            bpline(show, msgc_cancelled, "%s is in no shape for jumping.",
                   Monnam(u.usteed));
        else
            bpline(show, msgc_cancelled,
        ...
        bpline(show, msgc_cancelled, "%s is stuck in a trap.",
               Monnam(u.usteed));
        return 0;
    ...
    if (!checking) {
        if (you)
            bpline(show, msgc_uiprompt, "Where do you want to jump?");
        cc->x = ox;
        cc->y = oy;
    ...
    if (!isok(cc->x, cc->y)) {
        bpline(show, msgc_cancelled, "You cannot jump there!");
        return 0;
    } else if (!magic && !(trinsic & EXTRINSIC) &&
               dist2(ox, oy, cc->x, cc->y) != 5) {
        bpline(show, dist2(ox, oy, cc->x, cc->y) > 20 ? msgc_mispaste :
               msgc_cancelled, "Illegal move!");
        return 0;
    }
    ...
    if (dist2(ox, oy, cc->x, cc->y) > dist) {
        bpline(show, dist2(ox, oy, cc->x, cc->y) > 20 ? msgc_mispaste :
               msgc_cancelled, "Too far!");
        return 0;
    } else if (!m_cansee(mon, cc->x, cc->y)) {
        bpline(show, msgc_cancelled, "You cannot see where to land!");
        return 0;
    }
    ...
    if (you ? u.utrap : mon->mtrapped) {
        ...
        switch (traptype) {
        case TT_BEARTRAP:
            bpline(show, you ? msgc_badidea : msgc_monneutral,
            ...
            break;
        case TT_PIT:
            bpline(show, msgc_actionok,  "You leap from the pit!");
            break;
        case TT_WEB:
            bpline(show, msgc_actionok,
            ...
            break;
        case TT_LAVA:
            bpline(show, msgc_actionok, "%s %sself above the lava!",
...
use_trap(struct obj *otmp, const struct nh_cmd_arg *arg)
{
    ...
    if (nohands(youmonst.data))
        ...
        what = (youmonst.mx == level->dnladder.sx ||
                youmonst.mx == level->upladder.sx) ? "on the ladder" : "on the stairs";
    else if (IS_FURNITURE(level->locations[youmonst.mx][youmonst.my].typ) ||
             IS_ROCK(level->locations[youmonst.mx][youmonst.my].typ) ||
             closed_door(level, youmonst.mx, youmonst.my) || t_at(level, youmonst.mx, youmonst.my))
    ...
    if (otmp == u.utracked[tos_trap] &&
        youmonst.mx == u.utracked_location[tl_trap].x &&
        youmonst.my == u.utracked_location[tl_trap].y) {
        if (turnstate.continue_message)
            pline(msgc_occstart, "You resume setting %s %s.", shk_your(otmp),
                  trapexplain[what_trap(ttyp, youmonst.mx, youmonst.my, rn2) - 1]);
        one_occupation_turn(set_trap, occutext, occ_trap);
        return 1;
    }
    u.utracked[tos_trap] = otmp;
    u.utracked_location[tl_trap].x = youmonst.mx;
    u.utracked_location[tl_trap].y = youmonst.my;
    tmp = ACURR(A_DEX);
...
set_trap(void)
{
    ...
    ttyp = (otmp->otyp == LAND_MINE) ? LANDMINE : BEAR_TRAP;
    ttmp = maketrap(level, youmonst.mx, youmonst.my, ttyp, rng_main);
    if (ttmp) {
        ttmp->tseen = 1;
        ttmp->madeby_u = 1;
        newsym(youmonst.mx, youmonst.my);     /* if our hero happens to be invisible */
        if (*in_rooms(level, youmonst.mx, youmonst.my, SHOPBASE)) {
            add_damage(youmonst.mx, youmonst.my, 0L); /* schedule removal */
        }
        pline(msgc_actionok, "You finish arming %s.",
              the(trapexplain[what_trap(ttyp, youmonst.mx, youmonst.my, rn2) - 1]));
...
use_whip(struct obj *obj, const struct nh_cmd_arg *arg)
{
    ...
    if (Engulfed) {
        ...
        pline(msgc_yafm, "You flick a bug off of the %s.", ceiling(youmonst.mx, youmonst.my));
        ...
        if (levitates(&youmonst) || u.usteed) {
            ...
            if (otmp && proficient) {
                pline(msgc_occstart,
                      ...
                      an(singular(otmp, xname)), surface(youmonst.mx, youmonst.my));
...
find_polearm_target(int minr, int maxr, coord *cc)
{
    ...
    int x, y;
    for (x = (youmonst.mx - 2); x <= (youmonst.mx + 2); x++) {
        for (y = (youmonst.my - 2); y <= (youmonst.my + 2); y++) {
...
use_grapple(struct obj *obj, const struct nh_cmd_arg *arg)
{
    ...
    case 2:    /* Monster */
        ...
        if (verysmall(mtmp->data) && !rn2(4) &&
            enexto(&cc, level, youmonst.mx, youmonst.my, NULL)) {
    ...
    case 3:    /* Surface */
        if (IS_AIR(level->locations[cc.x][cc.y].typ) ||
            ...
            pline(msgc_actionok, "You are yanked toward the %s!",
                  surface(cc.x, cc.y));
            hurtle(sgn(cc.x - youmonst.mx), sgn(cc.y - youmonst.my), 1, FALSE);
            spoteffects(TRUE);
        }
        return 1;
