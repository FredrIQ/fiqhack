...
clear_monster_object_knowledge(struct obj *chain)
{
    ...
    for (obj = chain; obj; obj = obj->nobj) {
        if (Has_contents(obj))
            clear_monster_object_knowledge(obj->cobj);
...
can_make_bones(d_level *lev)
{
    ...
    /* Don't make bones in the endgame */
    if (depth(lev) <= 0)
        return FALSE;

    /* Everything else should allow bones in wizmode */
    if (wizard)
        return TRUE;

    /* Make bones less common on shallow depths */
    if (!rn2chance)
        return FALSE;
...
savebones(struct obj *corpse, boolean take_items)
{
    ...
    * reproduced correclty so as to get the same bones. */
    ...
    /* Don't leave any stale object memories */
    free_memobj();
...
make_bones:
    ...
    if (mtmp) {
        ...
        mtmp->mac = youmonst.mac + u.ublessed;
        mtmp->pw = youmonst.pw;
