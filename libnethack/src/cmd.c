...
#define ARG(c) { .cmdarg = (c) }
#define MUSE(c) { .cmdmuse = (c) }
#define NOARG { .cmdarg = NULL }
...
const struct cmd_desc cmdlist[] = {
    ...
    {"adjust", "adjust inventory letters", M('a'), 0, TRUE, ARG(doorganize),
     CMD_ARG_OBJ | CMD_EXT},
    {"annotate", "name the current level", 0, 0, TRUE, ARG(donamelevel),
     CMD_ARG_STR | CMD_EXT | CMD_MAINMENU},
    {"apply", "use a tool or container or dip into a potion", 'a', 0, FALSE,
     ARG(doapply), CMD_ARG_OBJ | CMD_ARG_POS | CMD_ARG_DIR | CMD_ARG_STR },
    {"attributes", "show status of your character", C('x'), 0, TRUE,
     ARG(doattributes), CMD_MAINMENU | CMD_NOTIME},
    ...
    {"conduct", "list status of voluntary challenges", 0, 0, TRUE,
     ARG(doconduct), CMD_EXT | CMD_NOTIME},
    {"countgold", "show gold, debt, credit, and unpaid items", '$', 0, TRUE,
     ARG(doprgold), CMD_NOTIME},
    {"dip", "dip an object into something", M('d'), 0, FALSE, ARG(dodip),
     CMD_EXT | CMD_ARG_OBJ},
    ...
    {"eat", "eat an item from inventory or the floor", 'e', 0, FALSE,
     ARG(doeat), CMD_ARG_OBJ},
    {"elbereth", "write an Elbereth in the dust", C('e'), 0, FALSE,
     ARG(doelbereth), 0},
    {"enhance", "advance or check your character's skills", M('e'), 0, TRUE,
     ARG(enhance_weapon_skill), CMD_EXT | CMD_MAINMENU},
    {"enhanceview", "check your character's skills", 0, 0, TRUE,
     ARG(enhance_weapon_skill_notime), CMD_NOTIME},
    {"engrave", "write on the floor", 'E', 0, FALSE, ARG(doengrave),
     CMD_ARG_OBJ | CMD_ARG_STR},
    {"equip", "change your equipment", 'A', '*', FALSE, ARG(doequip),
     CMD_MAINMENU},
    {"farlook", "say what is on a distant square", ';', 0, TRUE,
     ARG(doquickwhatis), CMD_NOTIME | CMD_ARG_POS},
    ...
    {"grope", "feel around for things at your feet", C('g'), M('g'),
     FALSE, ARG(dofeel), 0},
    {"history", "show a list of your historic deeds", 0, 0, TRUE,
     ARG(dohistory), CMD_EXT | CMD_NOTIME},
    {"idcheck", "check what an object can be", 'I', 0, TRUE, ARG(doidcheck),
     CMD_NOTIME},
    {"idtrap", "identify a trap", '^', 0, TRUE, ARG(doidtrap),
     CMD_ARG_DIR | CMD_NOTIME},
    {"inventory", "show your inventory", 'i', 0, TRUE, ARG(ddoinv),
     CMD_NOTIME},
    {"invoke", "invoke an object's powers", 'V', M('i'), TRUE,
     ARG(doinvoke), CMD_EXT | CMD_ARG_OBJ},
    {"jump", "jump to a location", M('j'), 'j', FALSE, ARG(dojump),
     CMD_ARG_POS | CMD_EXT},
    {"kick", "kick an adjacent object or monster", C('d'), 'k', FALSE,
     ARG(dokick), CMD_ARG_DIR},
    {"license", "show the NetHack license", 0, 0, TRUE, ARG(dolicense),
     CMD_HELP | CMD_NOTIME},
    {"lookhere", "describe the current square", ':', 0, TRUE, ARG(dolook),
     CMD_NOTIME},
    {"loot", "loot a bag or box on the floor", M('l'), 'l', FALSE,
     ARG(doloot), CMD_EXT},
    ...
    {"namemon", "christen a monster", 0, 0, TRUE, ARG(do_mname),
     CMD_ARG_POS | CMD_ARG_STR},
    {"nameitem", "name an item", 0, 0, TRUE, ARG(do_oname),
     CMD_ARG_OBJ | CMD_ARG_STR},
    {"nametype", "name a type of objects", 0, 0, TRUE, ARG(do_tname),
     CMD_ARG_OBJ | CMD_ARG_STR},
    {"offer", "offer a sacrifice to the gods", 0, 0, FALSE,
     ARG(dosacrifice), CMD_EXT | CMD_ARG_OBJ},
    {"open", "open, close or unlock a door", 'o', 0, FALSE, ARG(doopen),
     CMD_ARG_DIR},
    {"overview", "show an overview of the dungeon", C('o'), 0, TRUE,
     ARG(dooverview), CMD_EXT | CMD_NOTIME | CMD_MAINMENU},
    {"pay", "pay a shopkeeper", 'p', 0, FALSE, ARG(dopay),
     CMD_ARG_OBJ},
    {"pickup", "take items from the floor", ',', 0, FALSE, ARG(dopickup),
     CMD_ARG_LIMIT},
    {"pray", "pray to the gods for help", M('p'), 0, TRUE, MUSE(dopray),
     CMD_EXT | CMD_MUSABLE},
    {"quit", "give up on the current game", M('q'), 0, TRUE, ARG(doquit),
     CMD_EXT},
    {"quiver", "ready an item for firing", 'Q', 0, FALSE, ARG(dowieldquiver),
     CMD_ARG_OBJ},
    {"read", "read a scroll or spellbook", 'r', 0, FALSE, ARG(doread),
     CMD_ARG_OBJ},
    {"redraw", "redraw the screen", C('r'), 0, TRUE, ARG(doredrawcmd),
     CMD_NOTIME},
    {"repeat", "repeat the last command or continue an interrupted command",
     C('a'), 0, TRUE, NOARG, 0},
    {"ride", "ride (or stop riding) a monster", M('r'), 0, FALSE, ARG(doride),
     CMD_ARG_DIR | CMD_EXT},
    {"rub", "rub a lamp or a stone", 0, 0, FALSE, ARG(dorub),
     CMD_EXT | CMD_ARG_OBJ},
    {"sacrifice", "offer a sacrifice to the gods", M('o'), 0, FALSE,
     ARG(dosacrifice), CMD_EXT | CMD_ARG_OBJ},
    {"search", "search for hidden doors and traps", 's', 0, TRUE, ARG(dosearch),
     CMD_ARG_LIMIT},
    {"showamulets", "list your worn amulet", '"', 0, TRUE,
     ARG(dopramulet), CMD_NOTIME},
    {"showarmor", "list your worn armor", '[', 0, TRUE, ARG(doprarm),
     CMD_NOTIME},
    {"showrings", "list your worn rings", '=', 0, TRUE, ARG(doprring),
     CMD_NOTIME},
    {"showtools", "list your worn eyewear", '(', 0, TRUE, ARG(doprtool),
     CMD_NOTIME},
    {"showweapon", "list your wielded weapons", ')', 0, TRUE, ARG(doprwep),
     CMD_NOTIME},
    {"showworn", "list your equipment", 0, 0, TRUE, ARG(doprinuse),
     CMD_NOTIME},
    {"sit", "sit down", M('s'), 0, FALSE, ARG(dosit), CMD_EXT},
    ...
    {"spellbookview", "display your magical abilities", 0, 0, TRUE,
     ARG(dovspell_notime), CMD_NOTIME},
    ...
    {"takeoff", "take off an item you are wearing", 'T', 'R', FALSE,
     ARG(dounequip), CMD_ARG_OBJ},
    {"teleport", "use intrinsic or magical teleportation ability", C('t'), 0,
     TRUE, MUSE(dotele), CMD_MUSABLE},
    {"throw", "throw an item", 't', 0, FALSE, ARG(dothrow),
     CMD_ARG_OBJ | CMD_ARG_DIR | CMD_ARG_LIMIT},
    {"travel", "walk until a given square is reached", '_', 0, TRUE,
     ARG(dotravel), CMD_ARG_POS},
    {"turn", "turn undead", M('t'), 0, TRUE, MUSE(doturn),
     CMD_EXT | CMD_MUSABLE},
    {"twoweapon", "toggle two-weapon combat", M('2'), 'X', FALSE,
     ARG(dotwoweapon), CMD_EXT},
    {"untrap", "untrap something", M('u'), 'u', FALSE, ARG(dountrap),
     CMD_EXT},
    {"version", "displays the version number", 0, 0, TRUE, ARG(doversion),
     CMD_HELP | CMD_NOTIME | CMD_EXT},
    {"verhistory", "display the version history", 0, 0, TRUE, ARG(doverhistory),
     CMD_HELP | CMD_NOTIME | CMD_EXT},
    {"wait", "do nothing for one turn", '.', 0, TRUE, ARG(donull),
     CMD_ARG_LIMIT},
    {"wear", "wear clothing, armor, or accessories", 'W', 'P', FALSE,
     ARG(dowear), CMD_ARG_OBJ},
    {"wield", "hold an item in your hands", 'w', 0, FALSE, ARG(dowield),
     CMD_ARG_OBJ},
    {"wipe", "wipe off your face", M('w'), 0, FALSE, ARG(dowipe),
     CMD_EXT},
    ...
    {"moveonly", "move, but don't fight or pick anything up", 'm', 0,
     FALSE, ARG(domovecmd_nopickup), CMD_ARG_DIR | CMD_MOVE | CMD_ARG_LIMIT},
    {"run", "run, interacting with things found on the way", 0, 0, FALSE,
     ARG(dorun), CMD_ARG_DIR | CMD_MOVE | CMD_ARG_LIMIT},
    {"go", "run until something happens, cautiously", 'g', 'G', FALSE,
     ARG(dogo), CMD_ARG_DIR | CMD_MOVE | CMD_ARG_LIMIT},

    {"welcome", "(internal) display the 'welcome back!' message", 0, 0, TRUE,
     ARG(dowelcome), CMD_INTERNAL},
    {"options", "(internal) log a change of game options", 0, 0,
     TRUE, ARG(dointerrupt), CMD_INTERNAL},
    {"interrupt", "(internal) cancels a multi-turn command", 0, 0, TRUE,
     ARG(dointerrupt), CMD_INTERNAL},
    {"servercancel", "(internal) reread the replay, someone moved", 0, 0, TRUE,
     ARG(doservercancel), CMD_INTERNAL | CMD_NOTIME},

    {"desync", "(DEBUG) corrupt the save file", 0, 0, TRUE, ARG(wiz_desync),
     CMD_DEBUG | CMD_EXT},
    {"detect", "(DEBUG) detect unseen", 0, 0, TRUE, ARG(wiz_detect),
     CMD_DEBUG | CMD_EXT},
    {"genesis", "(DEBUG) create a monster", 0, 0, TRUE, ARG(wiz_genesis),
     CMD_DEBUG | CMD_ARG_LIMIT | CMD_ARG_STR | CMD_EXT},
    {"hpset", "(DEBUG) refill or change your current HP", 0, 0, TRUE,
     ARG(wiz_hpset), CMD_DEBUG | CMD_EXT | CMD_ARG_LIMIT},
    {"identify", "(DEBUG) identify all items in the inventory", C('i'), 0, TRUE,
     ARG(wiz_identify), CMD_DEBUG | CMD_EXT},
    {"impossible", "(DEBUG) test nonfatal error handling", 0, 0, TRUE,
     ARG(wiz_impossible), CMD_DEBUG | CMD_EXT},
    {"levelchange", "(DEBUG) change experience level", 0, 0, TRUE,
     ARG(wiz_level_change), CMD_DEBUG | CMD_EXT},
    {"levelcide", "(DEBUG) kill all other monsters on the level", 0, 0, TRUE,
     ARG(wiz_levelcide), CMD_DEBUG | CMD_EXT},
    {"lightsources", "(DEBUG) show mobile light sources", 0, 0, TRUE,
     ARG(wiz_light_sources), CMD_DEBUG | CMD_EXT | CMD_NOTIME},
    {"levelteleport", "(DEBUG) telport to a different level", C('v'), 0, TRUE,
     ARG(wiz_level_tele), CMD_DEBUG},
    {"monlist", "(DEBUG) show monster list", 0, 0, TRUE,
     ARG(wiz_mon_list), CMD_DEBUG | CMD_EXT},
    {"monpolycontrol", "(DEBUG) control monster polymorphs", 0, 0, TRUE,
     ARG(wiz_mon_polycontrol), CMD_DEBUG | CMD_EXT},
    {"panic", "(DEBUG) test fatal error handling", 0, 0, TRUE,
     ARG(wiz_panic), CMD_DEBUG | CMD_EXT},
    {"polyself", "(DEBUG) polymorph self", 0, 0, TRUE, ARG(wiz_polyself),
     CMD_DEBUG | CMD_EXT},
    {"printdungeon", "(DEBUG) print dungeon structure", 0, 0, TRUE,
     ARG(wiz_where), CMD_DEBUG | CMD_NOTIME | CMD_EXT},
    {"rewind", "(DEBUG) permanently undo gamestate changes", 0, 0, TRUE,
     ARG(wiz_rewind), CMD_DEBUG | CMD_NOTIME | CMD_EXT},
    {"seenv", "(DEBUG) show seen vectors", 0, 0, TRUE, ARG(wiz_show_seenv),
     CMD_DEBUG | CMD_EXT | CMD_NOTIME},
    {"showmap", "(DEBUG) reveal the entire map", 0, 0, TRUE, ARG(wiz_map),
     CMD_DEBUG | CMD_EXT},
    {"stats", "(DEBUG) show memory statistics", 0, 0, TRUE,
     ARG(wiz_show_stats), CMD_DEBUG | CMD_EXT | CMD_NOTIME},
    {"timeout", "(DEBUG) look at timeout queue", 0, 0, TRUE,
     ARG(wiz_timeout_queue), CMD_DEBUG | CMD_EXT | CMD_NOTIME},
    {"toggledetect", "(DEBUG) toggle intrinsic monster detection", 0, 0, TRUE,
     ARG(wiz_toggledetect), CMD_DEBUG | CMD_EXT},
    {"togglegen", "(DEBUG) toggle monster generation", 0, 0, TRUE,
     ARG(wiz_togglegen), CMD_DEBUG | CMD_EXT},
    {"vision", "(DEBUG) show vision array", 0, 0, TRUE, ARG(wiz_show_vision),
     CMD_DEBUG | CMD_EXT | CMD_NOTIME},
...
dowelcome(const struct nh_cmd_arg *arg)
{
    ...
    if (flags.easy)
        pline(msgc_intro, "You are in a mostly safe place, don't worry.");
...
{
    int res;
    boolean musable = !!(cmdlist[command].flags & CMD_MUSABLE);
    ...
    if (!musable && !cmdlist[command].cmdarg) {
        command = flags.last_cmd;
        arg = &flags.last_arg;
        turnstate.continue_message = FALSE;
        musable = !!(cmdlist[command].flags & CMD_MUSABLE);
    ...
    if (u.uburied && !cmdlist[command].can_if_buried) {
        ...
        res = (*cmdlist[command].cmdmuse) (&m);
    } else
        res = (*cmdlist[command].cmdarg) (arg);
