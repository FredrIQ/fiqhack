...
const struct cmd_desc cmdlist[] = {
    ...
    {"cast", "use a magical or supernatural ability", 'Z', 0, TRUE, ARG(docast),
    ...
    {"conduct", "list status of voluntary challenges", 0, 0, TRUE, ARG(doconduct),
     CMD_EXT | CMD_NOTIME},
    ...
    {"drop", "drop one item", 'd', 0, FALSE, ARG(dodrop),
     CMD_ARG_OBJ},
    {"eat", "eat an item from inventory or the floor", 'e', 0, FALSE, ARG(doeat),
     CMD_ARG_OBJ},
    {"elbereth", "write an Elbereth in the dust", C('e'), 0, FALSE, ARG(doelbereth),
     0},
    ...
    {"equip", "change your equipment", 'A', '*', FALSE, ARG(doequip), CMD_MAINMENU},
    {"farlook", "say what is on a distant square", ';', 0, TRUE, ARG(doquickwhatis),
    ...
    {"open", "open, close or unlock a door", 'o', 0, FALSE, ARG(doopen),
     CMD_ARG_DIR},
    {"overview", "show an overview of the dungeon", C('o'), 0, TRUE, ARG(dooverview),
    ...
    {"pray", "pray to the gods for help", M('p'), 0, TRUE, MUSE(dopray),
     CMD_EXT | CMD_MUSABLE},
    {"quit", "don't use this, use 'save' instead", M('q'), 0, TRUE, ARG(doquit),
     CMD_EXT | CMD_NOTIME},
    {"quiver", "ready an item for firing", 'Q', 0, FALSE, ARG(dowieldquiver),
     CMD_ARG_OBJ},
    {"read", "read a scroll or spellbook", 'r', 0, FALSE, ARG(doread), CMD_ARG_OBJ},
    ...
    {"throw", "throw an item", 't', 0, FALSE, MUSE(dothrow),
     CMD_ARG_OBJ | CMD_ARG_DIR | CMD_ARG_LIMIT | CMD_MUSABLE},
    {"travel", "walk until a given square is reached", '_', 0, TRUE, ARG(dotravel),
     CMD_ARG_POS},
    {"turn", "turn undead", M('t'), 0, TRUE, MUSE(doturn), CMD_EXT | CMD_MUSABLE},
    {"twoweapon", "toggle two-weapon combat", M('2'), 'X', FALSE, ARG(dotwoweapon),
     CMD_EXT},
    ...
    {"verhistory", "display the version history", 0, 0, TRUE, ARG(doverhistory),
     CMD_HELP | CMD_NOTIME | CMD_EXT},
    {"wait", "do nothing for one turn", '.', 0, TRUE, ARG(donull), CMD_ARG_LIMIT},
    {"wear", "wear clothing, armor, or accessories", 'W', 'P', FALSE, ARG(dowear),
     CMD_ARG_OBJ},
    ...
    {"run", "run, interacting with things found on the way", 0, 0, FALSE, ARG(dorun),
     CMD_ARG_DIR | CMD_MOVE | CMD_ARG_LIMIT},
    {"go", "run until something happens, cautiously", 'g', 'G', FALSE, ARG(dogo),
     CMD_ARG_DIR | CMD_MOVE | CMD_ARG_LIMIT},
    ...
    {"polyself", "(DEBUG) polymorph self", 0, 0, TRUE, ARG(wiz_polyself),
     CMD_DEBUG | CMD_EXT},
    {"printdungeon", "(DEBUG) print dungeon structure", 0, 0, TRUE, ARG(wiz_where),
    ...
    {"showmap", "(DEBUG) reveal the entire map", 0, 0, TRUE, ARG(wiz_map),
     CMD_DEBUG | CMD_EXT},
    {"stats", "(DEBUG) show memory statistics", 0, 0, TRUE, ARG(wiz_show_stats),
     CMD_DEBUG | CMD_EXT | CMD_NOTIME},
    {"timeout", "(DEBUG) look at timeout queue", 0, 0, TRUE, ARG(wiz_timeout_queue),
     CMD_DEBUG | CMD_EXT | CMD_NOTIME},
...
{
    boolean musable = FALSE;
    if (cmdlist[command].flags & CMD_MUSABLE)
        musable = TRUE;
    int res;
