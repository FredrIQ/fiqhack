...
static boolean mspell_would_be_useless(
  struct monst *mtmp, struct monst *mdef, unsigned int adtyp, int spellnum)
{
    if (adtyp == AD_SPEL) {
	/* haste self when already fast */
	if (mtmp->permspeed == MFAST && spellnum == MGC_HASTE_SELF)
...
static boolean uspell_would_be_useless(unsigned int adtyp, int spellnum)
{
    if (adtyp == AD_SPEL) {
	/* aggravate monsters, etc. won't be cast by peaceful monsters */
 ...
 * 0: unsuccessful spell
 */
int castmm(struct monst *mtmp, struct monst *mdef, const struct attack *mattk)
{
	int dmg, ml = mtmp->m_lev;
	int ret;
	int spellnum = 0;
...
/*
   ...
   and spell_would_be_useless().
 */
static void ucast_wizard_spell(
  struct monst *mattk, struct monst *mtmp, int dmg, int spellnum)
{
	    ...
	    for (i = rnd(tmp); i > 0; --i)
	        for(j=0; j<20; j++) {
	            ...
	            do {
                    ...
                    if (yours &&
		        !enexto(&bypos, level, u.ux, u.uy, &mons[makeindex]))
		        continue;
                    if (!yours &&
		        !enexto(&bypos, level, mattk->mx, mattk->my, &mons[makeindex]))
		        continue;
...
static void ucast_cleric_spell(
  struct monst *mattk, struct monst *mtmp, int dmg, int spellnum)
{
    boolean yours = (mattk == &youmonst);
