...
/* Last modified by Fredrik Ljungdahl, 2020-09-02 */
...
mdamagem(struct monst *magr, struct monst *mdef, const struct attack *mattk)
{
    ...
    case AD_PLYS:
        ...
        break;
    case AD_SLOW:
        if (!cancelled(magr) && !rn2(4) && !resists_slow(mdef) &&
    ...
    case AD_CURS:
        if (!cancelled(magr) && !rn2(10)) {
            if (pd == &mons[PM_CLAY_GOLEM]) {
                ...
                if (prop)
                    set_property(magr, prop, 0, FALSE);
            }
        }
        break;
    case AD_SGLD: {
