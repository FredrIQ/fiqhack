...
movemon(void)
{
    ...
    for (mtmp = level->monlist; mtmp; mtmp = nmtmp) {
        ...
        /* Special handling for vault guards */
        if (mx_egd(mtmp) && (DEADMONSTER(mtmp) || mtmp->mx == COLNO)) {
            /* This handles the fakecorr. TODO: Saner call chain. */
            gd_move(mtmp);
            continue;
        }
...
mondead(struct monst *mtmp)
{
    ...
    if (player_aware) {
        ...
        if (mtmp->summoned && cansuspectmon(mtmp))
 ...
 */
{
    ...
    if (corpse_chance(mtmp, NULL, FALSE) &&
        (!mx_egd(mtmp) || isok(mtmp->mx, mtmp->my)))
        make_corpse(mtmp);
    ...
    /* Vault guards might be on an invalid location. If others are, we want to
       know about it. */
    if (redisp && (!mx_egd(mtmp) || isok(x, y)))
        newsym(x, y);
