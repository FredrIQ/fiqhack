			...
			if (obj->lamplit)
                ...
                if (ignitable(obj) && obj->known &&
                    ...
                    long timeout = obj->lamplit ?
                        report_timer(obj->olev, BURN_OBJECT, (void*)obj) :
                        moves;
                    /* obj->age is the fuel remaining when the timer
                       runs out. So we add it to the turns the timer has
                       remaining to get remaining charge count. */
