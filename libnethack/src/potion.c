...
/* Last modified by Fredrik Ljungdahl, 2022-06-10 */
...
dodrink(const struct musable *m)
{
    ...
    if (strangled(mon)) {
        pline(msgc_cancelled,
              "If %s can't breathe air, how can %s drink liquid?",
              mon_nam(mon), mhe(mon));
        return 0;
    }
    ...
    potion = mgetargobj(m, fountain || sink ?
                        beverages_and_fountains : beverages, "drink");
    if (!potion)
        return 0;
    ...
    if (!you) {
        if (vis) {
            ...
            pline(combat_msgc(mon, NULL, cr_hit), "%s %s!",
                  M_verbs(mon, "drink"), singular(potion, doname));
        } else
...
peffects(struct monst *mon, struct obj *otmp, int *nothing, int *unkn)
{
    ...
    case POT_SLEEPING:
        ...
        if (you || vis)
            pline(statusbad, "%s suddenly fall%s asleep!", Mon, you ? "" : "s");
...
dodip(const struct nh_cmd_arg *arg)
{
    ...
    if (potion == &zeroobj) {
        if (IS_FOUNTAIN(here))
            ...
            water_damage(obj, NULL, TRUE); /* takes care of acid */
        return 1;
    }
