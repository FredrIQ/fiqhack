               ...
               unsigned reasons, boolean even_if_blocked)
{
    ...
    if (!(rv & W_MASK(os_cache))) {
        ...
        if ((property == BLINDED && !haseyes(mon->data)) ||
            ...
            (property == LIFESAVED && mon->summoned) ||
...
msensem(const struct monst *viewer, const struct monst *viewee)
{
    ...
    int viewerhostility = !!viewer->mpeaceful;
    int vieweehostility = !!viewee->mpeaceful;
    if (viewer->mtame || viewer == &youmonst)
        viewerhostility = 2;
    if (viewee->mtame || viewee == &youmonst)
        vieweehostility = 2;
