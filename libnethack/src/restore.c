...
restmonchn(struct memfile *mf, struct level *lev, boolean ghostly)
{
    ...
    while (count--) {
        ...
        if (flags.save_revision < 5)
            initialize_mon_pw(mtmp, rng_main);
...
restgamestate(struct memfile *mf)
{
    ...
    if (flags.save_revision >= 19) {
        restobjchn(mf, NULL, FALSE, FALSE, &gamestate.chest, NULL);
        if (gamestate.chest && gamestate.chest->ocontainer)
            gamestate.chest->ocontainer->cobj = gamestate.chest;
    }
...
dorecover(struct memfile *mf)
{
    ...
    /* set up olev on magic chest */
    setup_invent_olev(gamestate.chest);
