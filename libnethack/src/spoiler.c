...
{
    const char *alignbuf = "vs ";
    int aligns = 0;
    /* Other Relevant SPFX_ things: DBONUS */
    struct monclass mc = art->mtype;
    switch (mc.matchtyp) {
    case MTYP_ALL:
        return (mc.match) ? "vs all" : "vs none";
    case MTYP_PM:
        return msgprintf("vs %s", mons[mc.match].mname);
    case MTYP_S:
        return msgprintf("vs %c - %s", ((char) def_monsyms[mc.match]),
                         monexplain[(int) mc.match]);
    case MTYP_M1:
        return msgprintf("vs %s", spoilmonflagone(mc.match));
    case MTYP_M2:
        return msgprintf("vs %s", spoilmonflagtwo(mc.match));
    case MTYP_M3:
        return msgprintf("vs %s", spoilmonflagthree(mc.match));
    case MTYP_ALIGN:
#define VSALIGN(am, str)                                                \
    if (mc.match & am)                                                  \
        alignbuf = msgprintf("%s%s%s", alignbuf, aligns++ ? ", " : "",  \
                             str)                                       \

        VSALIGN(AM_LAWFUL, "lawful");
        VSALIGN(AM_NEUTRAL, "neutral");
        VSALIGN(AM_CHAOTIC, "chaotic");
        VSALIGN(AM_UNALIGNED, "unaligned");
        return alignbuf;
#undef VSALIGN
    default:
        return "N/A";
...
{
    short level = ((mskill >> (2 * proficiency)) & 3);
    if (level < 1) return "";
    ...
    if (change_fd_lock(fd, FALSE, LT_WRITE, 10)) {
        ...
        for (i = 0; mons[i].mlet; i++) {
            ...
            fprintf(f, "]\n");
            {
                ...
                fprintf(f, "   skills: [");
#define SKILL(a, b) j = ((pm->mskill >> (2 * a)) & 3);                  \
