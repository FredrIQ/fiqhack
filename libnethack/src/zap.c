...
bhitm(struct monst *magr, struct monst *mdef, struct obj *otmp, int range)
{
        ...
        if (wandlevel == P_MASTER &&
            ...
            if (otmp->ox == COLNO || selfzap) {
                ...
                if (magr == &youmonst) {
                    ...
                    tc.x = m_mx(mdef);
                    tc.y = m_my(mdef);
                    pline(msgc_uiprompt, "Teleport where?");
    ...
    case SPE_EXTRA_HEALING:
        ...
        if (mdef->data != &mons[PM_PESTILENCE]) {
            ...
            } else if (mdef == &youmonst || canseemon(mdef))
...
break_wand(struct monst *mon, struct obj *obj)
{
    ...
    explode(obj->ox, obj->oy, you ? 0 : -30, rnd(dmg), WAND_CLASS, expltype,
            NULL, 0);
 ...
 *   - BHIT_DMON       - a monster's displaced image was hit
