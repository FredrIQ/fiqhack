    ...
    if (IS_DOOR(typ) || typ == SDOOR) {
        ...
        case SPE_FORCE_BOLT:
            if (doorlock(obj, x, y)) {
                if (vis)
                    makeknown(obj->otyp);
                if (lev->locations[x][y].doormask == D_BROKEN &&
...
bhitm(struct monst *magr, struct monst *mdef, struct obj *otmp, int range)
{
    ...
    case SPE_STONE_TO_FLESH:
        ...
        if (selfzap) {
            ...
            /* It is possible that we can now merge some inventory. Do a highly paranoid
               merge. Restart from the beginning until no merges. */
            do {
                didmerge = FALSE;
    ...
    if (obj->oartifact) {
        ...
        if (vis)
            pline(msgc_substitute,
                  ...
                  The(xname(obj)));
        if (you)
            losehp(dice(obj->spe + 4, 8), killer_msg(DIED, "an outbursting wand"));
        else {
...
void
break_wand(struct monst *mtmp, struct obj *otmp)
{
    int i, x, y;
    int damage;
    int expltype;
    ...
    boolean oseen = (you || mon_visible(mtmp));

    otyp = otmp->otyp;
    otmp->ox = mtmp->mx;
    otmp->oy = mtmp->my;
    otmp->in_use = TRUE; /* don't allow destroy_item to mess with it */
    ...
    if (otyp == WAN_CREATE_MONSTER) {
        create_critters(otmp->spe, NULL, otmp->ox, otmp->oy);
        m_useup(mtmp, otmp);
        return;
    }

    /* affect all tiles around the monster */
    int shop_damage = 0; /* 1=digging, 2=striking/similar, 0=nothing or not yours */
    ...
    for (i = 0; i <= 8; i++) {
        ...
        if (otyp == WAN_DIGGING && dig_check(NULL, FALSE, x, y)) {
            ...
            digactualhole(x, y, NULL, (rn2(otmp->spe) < 3 ||
            ...
            int res_bhit = bhit_at(mtmp, otmp, x, y, 10);
...
dozap(const struct musable *m)
{
    ...
    if (!zappable(mon, obj)) {       /* zappable prints the message itself */
        ...
        return ((!you && DEADMONSTER(mon)) ? 2 : 1);
...
weffects(struct monst *mon, struct obj *obj, schar dx, schar dy, schar dz)
{
    ...
    if (you && u.usteed && (objects[otyp].oc_dir != NODIR) &&
        ...
        if (otyp == WAN_DIGGING || otyp == SPE_DIG)
            ...
            if (you)
                buzz(otyp - SPE_MAGIC_MISSILE + 10, youmonst.m_lev / divisor + 1,
                     youmonst.mx, youmonst.my, dx, dy, wandlvl);
            else
