...
#ifndef WIN32
#error !AIMAKE_FAIL_SILENTLY! wincon.c works on Windows systems only. Use tty.c instead.
#endif
...
int wincon_hook_getkeyorcodepoint(int timeout_ms) {
    ...
    /* Has the /window/ been resized? If so, and the user didn't explicitly
       set the buffer size, adapt to the new window size. */
...
recheck:
    ...
    while (!count) {
        ...
        if (inrecords[0].EventType != WINDOW_BUFFER_SIZE_EVENT &&
            inrecords[0].EventType != KEY_EVENT) count = 0;
        if (inrecords[0].EventType == KEY_EVENT &&
            !inrecords[0].Event.KeyEvent.bKeyDown) count = 0;
    }
    if (inrecords[0].EventType == WINDOW_BUFFER_SIZE_EVENT) {
        /* Someone resized the buffer. Possibly. */
        /* If the user's explicitly resizing the buffer, ignore the window
...
#define VKK(x,y) case VK_##x: lastkeyorcodepoint = KEY_BIAS + KEY_##y; break
        ...
        VKK(ADD,NUMPLUS); VKK(SUBTRACT,NUMMINUS); VKK(NUMLOCK,PF1);
        VKK(MULTIPLY,NUMTIMES); VKK(DIVIDE,NUMDIVIDE);
        ...
        if (lastkeyorcodepoint == 0) {
            if (kp->wVirtualKeyCode < 256)
                ...
                lastkeyorcodepoint = KEY_BIAS +
                    (KEY_FUNCTION | kp->wVirtualKeyCode);
            else
