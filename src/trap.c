...
static int steedintrap(struct trap *trap, struct obj *otmp)
{
	...
	switch (tt) {
		...
		case POLY_TRAP: 
			...
			newcham(mtmp, (struct permonst *)0,
				       FALSE, FALSE);
...
static int mkroll_launch(struct trap *ttmp, xchar x, xchar y,
...
static void domagictrap(void)
{
	...
	if (fate < 10) {
	  ...
	  while(cnt--)
		makemon((struct permonst *) 0, u.ux, u.uy, NO_MM_FLAGS);
	}
	else
	  switch (fate) {
