...
static int
write_padtile(pixel(*pixels)[MAX_TILE_X], const char *ttype,
              int number, const char *name)
{
    ...
    for (j = 0; j < tile_y; j++)
        for (i = 0; i < tile_x; i++)
            bigpixels[j + tile_y / 2][i + tile_x / 4] = pixels[j][i];
    i = tile_x;
    j = tile_y;
    tile_x = tile_x * 3 / 2;
    tile_y *= 2;
...
static int
write_slanttile(pixel(*pixels)[MAX_TILE_X], const char *ttype, int number,
                const char *name, int yoffset)
{
    int i, j;
    int retval;
    pixel bigpixels[MAX_TILE_Y][MAX_TILE_X];
