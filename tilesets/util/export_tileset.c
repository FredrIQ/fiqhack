...
static void
save_png(const char *filename, int width, int height)
{
    /* [AJA] all the volatiles here may seem strange.  They are needed because
       the ANSI C standard (which GCC adheres to) says that when setjmp/longjmp 
       is being used, only volatile local variables are guaranteed to keep
       their state if longjmp() gets called. */
    FILE *volatile fp = NULL;

    png_bytep *volatile row_pointers = NULL;

    /* we take the address of these two, so we shouldn't need the volatile.
       (GCC complains about discarding qualifiers if the volatile is there). */
    ...
    png_set_IHDR(png_ptr, info_ptr, width, height, 8,
                 tile_palette ? PNG_COLOR_TYPE_PALETTE : has_trans ?
    ...
    if (tile_palette)
        ...
        for (row = 0; row < height; row++)
            row_pointers[row] =
                tile_bytes_rgba + row * width * (has_trans ? 4 : 3);
    ...
    /* [AJA] Normally I don't like gotos.  In this situation where there are
       lots of points of possible failure and a growing set of things to be
       undone, it makes for nicer code. */
failed:
    if (png_ptr) {
    ...
    while (read_text_tile(tile)) {
        ...
        for (y = 0; y < tile_y; y++, tb += stride)
            for (x = 0; x < tile_x; x++) {
                ...
                if (has_trans)
                    tb[x * pix_w + 3] =
                        pixel_equal(tile[y][x], trans_p) ? 0 : 255;
                ...
                if (tile_bytes_i)
                    tile_bytes_i[((by * tile_y + y) * num_across +
                                  bx) * tile_x + x] = i;
            }
...
process_file(const char *fname)
{
    ...
    if (!tile_palette) {
        ...
        /* [ALI} PNG encoding is more efficient if the transparent colour is
           first in the palette, so add it here. This will cause a slight
           inefficiency if the image data proves not to contain any transparent 
           pixels, but still seems the best option. */
        if (has_trans) {
            tile_palette_n = 1;
