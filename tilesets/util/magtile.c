...
static int
write_magtile(pixel (*pixels)[MAX_TILE_X], const char *ttype,
    ...
    for (j = 0; j < tile_y; j++)
        for (i = 0; i < tile_x; i++) {
            ...
            /* Scale2X algorithm from AdvanceMAME
               http://advancemame.sourceforge.net/scale2x.html Pixel E from a
               source image B D E F H is magnified to a set of 4
               destination pixels E0 E1 E2 E3 by the following rules E0 = D
               == B && B != F && D != H ? D : E; E1 = B == F && B != D && F !=
               H ? F : E; E2 = D == H && D != B && H != F ? D : E; E3 = H == F
               && D != H && B != F ? F : E; */
            pixel pixB;
